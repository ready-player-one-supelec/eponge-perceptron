...
def doUrStuff(run, learning_rate):
    ...
    layer1 = Layer(2, 2, activation.regulated_tanh, learning_rate)
    layer2 = Layer(2, 1, activation.regulated_tanh, learning_rate)
    network = Network([layer1, layer2], False)

    for i in range(len(iterations)):
        error[i] += xor.main(step, iterations[i], network,
                             training_image, training_label, run, True, learning_rate)
    return error


def map_XOR():
    layer1 = Layer(2, 2, activation.regulated_tanh, 0.05)
    layer2 = Layer(2, 1, activation.regulated_tanh, 0.05)
    network = Network([layer1, layer2], False)
    for i in iterations:
...
def multicoreGraph():
    ...
    for j in learning_rates_tab:
        error = [0] * len(iterations)
        results = pool.starmap(doUrStuff, tuple(
            [(i, j) for i in range(1, runs+1)]))
