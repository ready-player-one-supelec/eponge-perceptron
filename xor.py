...
def main(step, iteration, network, training_image, training_label, run, do_test, learning_rate):
    ...
    print("Run #{} -".format(run), "Iteration :",
          iteration, " - Learning rate : ", learning_rate)
    for a in range(step):
        for e in liste:
...
def graph2D(errors, iterations, learning_rates, runs):
    ...
    for i in range(len(learning_rates)):
        plt.semilogy(
            iterations, errors[i], '-', label="Learning rate = {}".format(learning_rates[i]))
    plt.legend()
...
def graph3D(errors, iterations, learning_rates, runs, save_data):
    ...
    ax.set_zlabel(
        "Euclidian norm of the error (average with {} runs)".format(runs))
    ax.set_title("3D graph of the error for a XOR neural network")
    surf = ax.plot_surface(X, Y, Z, cmap=cm.coolwarm,
                           linewidth=0, antialiased=False)
    fig.colorbar(surf, shrink=0.5, aspect=5)
    if save_data:
        save.save_data_for_3D_XOR(
            errors, iterations, learning_rates, runs, "data/XOR/graph3D_data")
    else:
        plt.show()
